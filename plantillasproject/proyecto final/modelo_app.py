...
def alta_producto(producto, fecha, cantidad, precio, costo, ganancia, tree):
    patron = "^[A-Za-záéíóú ]*$"  # regex para el campo cadena
    if re.match(patron, producto):
        print(producto, fecha, cantidad, precio, costo, ganancia)
        con = conexion_abase()
        cursor = con.cursor()
        data = (producto, fecha, cantidad, precio, costo, ganancia)
        sql = "INSERT INTO productos(producto, fecha, cantidad, precio, costo, ganancia, ) VALUES(?, ?, ?, ?, ?, ? )"
...
def modificar(con, fecha, producto, cantidad, precio, costo):
    cursor = con.cursor()
    data = (fecha, producto, cantidad, precio, costo)
    sql = "UPDATE producto SET fecha =? producto =? cantidad =? precio =? WHERE fecha =?"
    cursor.execute(sql, data)
    con.commit()
    actualizar_treeview(tree)
...
fecha1 = Entry(panel, textvariable=var_fecha, width=25)
...
tree.column("col5", width=100, minwidth=50, anchor=W)
...
boton_guardar = Button(panel, text="Guardar", command=lambda: alta_producto(
    var_fecha.get(), variable.get(), variable1.get(), variable2.get(), variable3.get(), variable4.get(), tree))
